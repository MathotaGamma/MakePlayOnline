...
class Complex {
  ...
  round(k=0){
    let k_k;
    if(k >= 0){
      k_k = new Complex(this._real.toFixed(k),this._imag.toFixed(k));
    } else {
      k_k = new Complex(Math.round(this._real*10**k)/10**k,Math.round(this._imag*10**k)/10**k)
    }
    return k_k;
  }
  ...
  static _graph(id,origin_func,start,end,step,params={}){
    ...
    if(Number.isNaN(start) || Number.isNaN(end) || Number.isNaN(step)){
      throw new Error('Complex-Argument error->The arguments start, end and step of the _graph method must be real numbers.');
    }
    if(start > end || step <= 0){
      throw new Error('Complex-Argument error->The argument of the _graph method, end, must be a number greater than start, and step must be greater than or equal to 0.');
    }
...
throw new Error('Complex-Timeout error->Too long.');
          break;
        }
      }
    }
